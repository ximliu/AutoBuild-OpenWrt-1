...
- ### 理论上在openwrt-系统-备份/升级里能后台升级的机子都支持定时更新的，R2S/R4S这些用TF卡的就不支持，N1没测试，不过应该也是不支持

- ### 测试方法，分别在两个时间段启动编译，比如1:00启动一个编译,然后1:05分启动一个编译，完成后，安装1:00的，就会自动检测到1:05的
#
- ### 在build/机型文件夹/settings.ini，把 REGULAR_UPDATE="false" 设置成 REGULAR_UPDATE="true" 为开启把定时更新固件的插件编译进固件
